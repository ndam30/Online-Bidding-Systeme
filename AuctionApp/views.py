...
def Home(request):
    ...
    d = {'error':error,'data':data, 'cat':cat, 'pro':prod}
...
def Profile1(request):
    ...
    try:
        ...
        pro = None
        u = "trainer"
    d = {'pro':pro,'error':error,'data':data, 'u':u, 'count':count,'new2':new2}
...
def Add_Category(request):
    ...
    if request.method == 'POST':
        ...
        Category.objects.create(name=n,image=im)
        error = True
...
def delete_product(request,pid):
    pr = Aucted_Product.objects.get(id=pid)
    pr.delete()
...
def contact(request):
    if request.method == 'POST':
        ...
        send_mail(
            'Mr/Mme '+ name +' ',
            'Just Contacted LAMIDEV On The Number  '+ message +'.',
            settings.EMAIL_HOST_USER,
            ['lamboarmand28@gmail.com'],
            fail_silently=False
        )

        messages.success(request, 'Your request has been submitted, a LAMIDEV will get back to you soon')
    return render(request, 'contact.html')
